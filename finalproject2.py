...
#  = (width / 2, height / 2)create variables to store location and size of a shape to draw on screen.
...
borderwest = pygame.Rect(0, 0, 5, 700)
bordereast = pygame.Rect(945, 0, 5, 950)
bordernorth = pygame.Rect(0, 0, 950, 5)
bordersouth = pygame.Rect(0, 697.5, 945, 5)

for _ in range(enemy_count):
    pos_x = random.randint(circle_pos[0] + 100, width)
    pos_y = random.randint(circle_pos[1] + 100, height)
...
while True:
    ...
    if score >= 15:
        if move_right:
            circle_pos = (circle_pos[0] + 31 // 10, circle_pos[1])
        if move_left:
            circle_pos = (circle_pos[0] - 31 // 10, circle_pos[1])
        if move_down:
            circle_pos = (circle_pos[0], circle_pos[1] + 31 // 10)
        if move_up:
            circle_pos = (circle_pos[0], circle_pos[1] - 31 // 10)
    if score >= 30:
        if move_right:
            circle_pos = (circle_pos[0] + 31.5 // 10, circle_pos[1])
        if move_left:
            circle_pos = (circle_pos[0] - 31.5 // 10, circle_pos[1])
        if move_down:
            circle_pos = (circle_pos[0], circle_pos[1] + 31.5 // 10)
        if move_up:
            circle_pos = (circle_pos[0], circle_pos[1] - 31.5 // 10)

    if player_collision.colliderect(bordereast):
        player_alive = False
        circle_pos = (100, 100)
        print("GAME OVER")
        ...
        for i in range(len(enemy_positions)):
            pos_x = random.randint(700, width)
            pos_y = random.randint(200, height)
            enemy_positions [i] = pygame.Vector2(pos_x, pos_y)
        player_alive = True

    if player_collision.colliderect(borderwest):
        player_alive = False
        circle_pos = (100, 100)
        print("GAME OVER")
        print("Your score is now 0")
    ...
    if player_collision.colliderect(bordernorth):
        player_alive = False
        circle_pos = (100, 100)
        print("GAME OVER")
        print("Your score is now 0")
    ...
    if player_collision.colliderect(bordersouth):
        player_alive = False
        circle_pos = (100, 100)
        print("GAME OVER")
        print("Your score is now 0")
    ...
    if move_right:
        circle_pos = (circle_pos[0] + 30 // 10, circle_pos[1])
    if move_left:
        circle_pos = (circle_pos[0] - 30 // 10, circle_pos[1])
    if move_down:
        circle_pos = (circle_pos[0], circle_pos[1] + 30 // 10)
    if move_up:
        circle_pos = (circle_pos[0], circle_pos[1] - 30 // 10)
    ...
    screen.fill(shape_color, rect=bordereast)
    screen.fill(shape_color, rect=borderwest)
    screen.fill(shape_color, rect=bordernorth)
    screen.fill(shape_color, rect=bordersouth)
    ...
    if player_collision.colliderect(coin_collision):
        ...
        coin_pos = (random.randint(0, width - 20), random.randint(0, height- 20))
    ...
    for position in enemy_positions:
        ...
        direction_vector *= 0.1 * score + 0.5
